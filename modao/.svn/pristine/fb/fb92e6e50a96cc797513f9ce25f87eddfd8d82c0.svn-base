                    ...
                    public void onNext(BaseResponse<ResidentInfoBean> baseResponse) {
                        if (baseResponse.getStatus() == 1) {
                            ...
                            if (baseResponse.getStatus_name().contains("房屋不存在")) {
                                Toast.makeText(AddressBindActivity.this, "没有查到符合条件的地址", Toast.LENGTH_SHORT).show();
                            } else if (baseResponse.getStatus_name().contains("用户不存在")) {
                                Toast.makeText(AddressBindActivity.this, "用户不存在", Toast.LENGTH_SHORT).show();
                            } else if (baseResponse.getStatus_name().contains("房屋已绑定")) {
                                Toast.makeText(AddressBindActivity.this, "房屋已绑定", Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
                });
